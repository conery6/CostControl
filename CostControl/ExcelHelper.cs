...
namespace CostControl
{
	class ExcelHelper
	{
		...
		public void SetTitle(string title, int lastCol)
		{
			...
			cell.CellStyle = GetStyle(CellStyle.Title);
		...
		public void DataTableToExcel(DataTable dt, int rowStart, int colStart, bool hasHead = true)
		{
			...
			if (hasHead)
			{
				...
				for (int i = 0; i < dt.Columns.Count; i++)
				{
					...
					headCell.CellStyle = GetStyle(CellStyle.Header);
		...
		private void SetCellValue(ref ICell newCell, object obj)
		{
			...
			switch (t.ToString())
			{
				...
				case "System.DateTime"://日期类型
					...
					newCell.CellStyle = GetStyle(CellStyle.DateTime);//格式化显示
					break;
				...
				case "System.Double":
					...
					newCell.CellStyle = GetStyle(CellStyle.DoubleNumber);
					newCell.SetCellType(CellType.Numeric);
					break;
